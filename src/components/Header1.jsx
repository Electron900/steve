      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex items-center justify-between py-5">
          <div className="flex items-center space-x-4">
            <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${currentTheme.gradient} text-2xl shadow-2xl`}>
              {portfolioData.profile.avatar}
            </div>
            <div>
              <h1 className={`text-xl font-black ${currentTheme.text}`}>
                {portfolioData.profile.name}
              </h1>
              <p className={`text-xs ${currentTheme.textSecondary}`}>{portfolioData.profile.title}</p>
            </div>
          </div>
          ...
          >
            {theme === 'dark' ? <SunIcon className="h-5 w-5 text-yellow-400" /> : <MoonIcon className="h-5 w-5 text-blue-600" />}
          </button>
        </div>
